/*
 * Original code from https://github.com/jmharvey1/ArduinoCPO
 * 
 ...
 *********************************
 * Modifications by JH 20230719
 * Using Arduino Nano so A0 is on the same pin as D14 and A1 is on the same pin as D15
 * Changing pins as follows:
 * Old D14 = D4  (Speaker)
 * Old D15 = D5  (Speaker)
 * Old D16 = D6  (Straight Key)
 * 
 * Storing pins in variables for easier adaptability 
 * 
 */
...
void setup() {
  ...
  pinMode(ditButtonPin, INPUT_PULLUP);
  //pinMode(10, INPUT_PULLUP);
  pinMode(dahButtonPin, INPUT_PULLUP);
  
  //configure A2 (cwOutPin) as digital output [used in this case to drive a seperate CW decoder module]
  //pinMode(A2, OUTPUT);
  pinMode(cwOutPin, OUTPUT);
  ...
  pinMode(speedDecPin, INPUT_PULLUP);
  //digitalWrite(A0, HIGH); // Commented out by original author
  //pinMode(A1, INPUT_PULLUP);
  pinMode(speedIncPin, INPUT_PULLUP);
  //digitalWrite(A1, HIGH); // Commented out by original author
  DitPeriod = 1200/wpm;
...
void loop() {
  ...
  int Pin16Val = digitalRead(straightButtonPin);//straight Key input
  int Pin10Val = digitalRead(dahButtonPin); //Paddle "Dit" input
  int Pin11Val = digitalRead(ditButtonPin);//Paddle "Dah" input
  int PinA0Val = digitalRead(speedDecPin); //Speed Decrement input
  int PinA1Val = digitalRead(speedIncPin);//Speed Increment input
  if(spdSwNO){
    PinA0Val = !PinA0Val; 
    PinA1Val = !PinA1Val;
  }
    ...
    if (!SpaceActive){
      if (End > millis()){
       ...
       if (End > millis()){
        ...
        digitalWrite(cwOutPin, HIGH); //output to remote decoder
        //digitalWrite(14, LOW);
        digitalWrite(speaker1Pin, LOW);
        //digitalWrite(15, LOW);
        digitalWrite(speaker2Pin, LOW);
...
void MakeTone(){//Cycle the speaker pins
  ...
  digitalWrite(cwOutPin, LOW); //output to remote decoder
  //digitalWrite(14, LOW);
  digitalWrite(speaker1Pin, LOW);
  //digitalWrite(15, HIGH);
  digitalWrite(speaker2Pin, HIGH);
  delayMicroseconds(TonePeriod);
  //digitalWrite(14, HIGH);
  digitalWrite(speaker1Pin, HIGH);
  //digitalWrite(15, LOW);
  digitalWrite(speaker2Pin, LOW);
}
